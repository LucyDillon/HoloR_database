...
out="$PROJECTFOLDER/results/align_rumen/soft_trimmed"
...
bowtie2 -x "$index" \
    ...
    --local \
    2> "$out/${sample}_local_summary.txt" \
    | samtools sort -@ 4 -m 4G -T "$PROJECTFOLDER/tmp/${sample}_sort" -o "$out/${sample}_local.sorted.bam" -

samtools index "$out/${sample}_local.sorted.bam"